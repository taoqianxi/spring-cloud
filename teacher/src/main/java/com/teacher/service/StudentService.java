...
import com.teacher.util.RedisLockUtil;
import lombok.Data;
...
public class StudentService {
    ...
    public List<Student> studentList(){
        String lockKey = "lock001";
        RLock rLock = redissonClient.getFairLock(lockKey);
        try {
            rLock.lock(60 * 3, TimeUnit.SECONDS);
            System.out.println("-------------");
        } finally {
            rLock.unlock();
        }
    ...
    public static void main(String[] args) {
        Map<String,String> parse = JSON.parseObject(bankName, Map.class);
        BankBean bankBean = JSON.parseObject(getCardDetail("6228481269040908170"), BankBean.class);
        String s = parse.get(bankBean.getBank());
        System.err.println(s);
    }

    @Data
    private static class BankBean{
        private String cardType;//银行卡类型 DC为储蓄卡，CC为信用卡。
        private String bank; // 银行的名称代码
        private String key;
        private String messages;
        private Boolean validated; //是否通过验证
        private String stat;
    }
    /*
     * TODO
